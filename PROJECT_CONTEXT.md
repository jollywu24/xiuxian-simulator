...
## 当前开发方向：2D武侠沙盒 S0（2026-09-17）

用户当前目标已转为对标成熟武侠沙盒的2D体验。定稿与可玩样板入口见[沙盒S0交付](docs/sandbox/README.md)。`npm run serve:s0`启动独立工作台，源码在`tools/sandbox-s0/`，数据在`data/sandbox/`。正式`web/`仍是此前3D叙事版本，不能把样板能力描述为线上已上线。S0实现与文档已交付，A07目标设备及制作效率外部验证仍未关闭；详见阶段状态。后文旧系统导航继续用于维护现行版本。
...
| 十一类容貌状态、固定头脸锚点、分层部件、旧档回落与Canvas合成 | `web/appearance-core.mjs`、`web/paperdoll-system.mjs`、`web/paperdoll-renderer.mjs`、`scripts/build-appearance-rig-v5.py` | `tests/appearance-system.test.mjs`、`tests/paperdoll-system.test.mjs`、`scripts/cdp-appearance.mjs`、`docs/systems/12_APPEARANCE_RIG.md` |
