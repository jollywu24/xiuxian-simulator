...
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /破庙里，有人先你一步点了灯/);
assert.doesNotMatch(await evaluate(`document.querySelector("#app")?.innerText || ""`), /霜河帮主母/);
assert.equal(await evaluate(`document.querySelectorAll('[data-action="encounter-choice"]').length`), 3);
await screenshot("/tmp/taixu-encounter-desktop.png");
await send("Emulation.setDeviceMetricsOverride", { width: 390, height: 844, deviceScaleFactor: 1, mobile: true });
const mobileEncounter = await snapshot("encounter-mobile");
assert.ok(mobileEncounter.scrollWidth <= mobileEncounter.viewport[0]);
await screenshot("/tmp/taixu-encounter-mobile.png");
await send("Emulation.setDeviceMetricsOverride", { width: 1280, height: 720, deviceScaleFactor: 1, mobile: false });
await click("encounter-choice", "heal");
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /霜河帮主母/);
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /柳照影/);
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /辨出箭毒/);
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /锁息丹/);
await screenshot("/tmp/taixu-encounter-outcome.png");
await click("continue-after-encounter");
...
assert.match(await evaluate(`document.querySelector(".character-panel")?.innerText || ""`), /奇缘簿/);
assert.match(await evaluate(`document.querySelector(".character-panel")?.innerText || ""`), /锁息丹/);
await click("start-sim1");
assert.equal(await evaluate(`Boolean(document.querySelector('[data-action="sim-morning"][data-value="encounter-heal"]'))`), true);
await click("sim-morning", "encounter-heal");
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /真正的杀招一定在后面/);
assert.match(await evaluate(`document.querySelector(".character-panel")?.innerText || ""`), /丹药已服/);
...
assert.equal(saved.encounterChoiceId, "heal");
assert.equal(saved.encounterOutcome.boon.id, "breath_lock_pill");
...
await click("choose-companion", "wen");
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /答应同行/);
assert.match(await evaluate(`document.querySelector("#app")?.innerText || ""`), /毒物/);
await click("to-mine-approach");
