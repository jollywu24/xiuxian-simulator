...
assert.match(await evaluate(`document.querySelector('script[type="module"]')?.src || ""`), /wudao-app\.mjs\?v=20260716\.3/);
assert.match(await evaluate(`document.querySelector('link[rel="stylesheet"]')?.href || ""`), /styles\.css\?v=20260716\.3/);
...
const firstObserveCheck = JSON.parse(await evaluate(`localStorage.getItem("wudao-high-martial-v1")`)).p0.battle.lastCheck;
assert.equal(firstObserveCheck.tier, "great");
await reloadWithSave(battleStartSave);
await click("first-battle-action", "observe");
const repeatedObserveCheck = JSON.parse(await evaluate(`localStorage.getItem("wudao-high-martial-v1")`)).p0.battle.lastCheck;
...
failedCheckSave.fateSeed = "seed-3";
...
failedCheckSave.p0.battle = createFirstBattle();
...
assert.match(await text(), /肋下先被刀锋擦开/);
...
assert.equal(failedCheckState.p0.battle.round, 2);
...
assert.match(await text(), /一盏命灯碎裂/);
assert.match(await text(), /因果骰 · 失手/);
const repeatedFailureDeath = JSON.parse(await evaluate(`localStorage.getItem("wudao-high-martial-v1")`)).p0.deathRecords.at(-1);
assert.equal(repeatedFailureDeath.id, "failed_wrist_check");
assert.equal(repeatedFailureDeath.check.tier, "failure");
