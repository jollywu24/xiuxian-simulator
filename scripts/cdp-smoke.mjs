...
assert.match(await evaluate(`document.querySelector('script[type="module"]')?.src || ""`), /wudao-app\.mjs\?v=20260718\.3/);
assert.match(await evaluate(`document.querySelector('link[rel="stylesheet"]')?.href || ""`), /styles\.css\?v=20260718\.3/);
...
assert.match(await text(), /真正杀招藏在左袖|左袖反刺/);
...
await click("resolve-first-battle-enemy");
await click("resolve-first-battle-enemy");
await click("resolve-first-battle-enemy");
