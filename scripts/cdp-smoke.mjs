...
assert.match(await evaluate(`document.querySelector(".narrative-entry:last-child .player-choice")?.innerText || ""`), /痕迹|摆给她看/);
await evaluate(`document.querySelector(".narrative-deck").scrollTop = 0`);
await screenshot("wudao-narrative-history-desktop.png");
await click("temple-crisis", "drop_rack");
assert.match(await text(), /乌沉药匣/);
...
assert.equal(saved.relationship, "她愿意替你担保一次");
