...
await screenshot("wudao-origin-selection-reference-1672x941.png");
...
assert.match(await evaluate(`document.querySelector(".landscape-required")?.innerText || ""`), /请横置设备/);
await screenshot("wudao-landscape-required-phone-portrait.png");
await send("Emulation.setDeviceMetricsOverride", { width: 1280, height: 720, deviceScaleFactor: 1, mobile: false });
await evaluate(`new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)))`);
...
assert.ok(referenceOpening.sceneMarkers.positions.incense_rack[0] > 0.25 && referenceOpening.sceneMarkers.positions.incense_rack[0] < 0.28);
...
assert.equal(shortDesktopOpening.currentChoicesVisible, false);
...
assert.equal(scaledDesktopOpening.currentChoicesVisible, false);
...
assert.equal(phoneLandscapeOpening.currentChoicesVisible, false);
...
assert.equal(phoneLandscapeChoiceFlow.openingFeed.currentChoices, 0);
...
assert.equal(peachInventory.inventoryView.useDisabled, false);
assert.match(peachInventory.inventoryView.detail, /饥饿|果腹/);
...
await send("Emulation.setDeviceMetricsOverride", { width: 844, height: 390, deviceScaleFactor: 1, mobile: true });
const landscapeExploration = await snapshot("temple-exploration-landscape");
assert.ok(landscapeExploration.scrollWidth <= 844);
assert.equal(landscapeExploration.splitView, true);
assert.equal(landscapeExploration.dockCount, 4);
await screenshot("wudao-temple-exploration-landscape.png");
...
assert.deepEqual((await snapshot("character-head-unequipped-paperdoll")).characterView.paperDollItems, []);
...
await click("inspect-scene-object", "deity_statue");
await click("temple-object-action", "deity_statue|inspect_statue");
await click("temple-area", "rear");
await click("inspect-scene-object", "patched_wall");
...
const footstepsState = JSON.parse(await evaluate(`localStorage.getItem("wudao-high-martial-v1")`));
assert.equal(footstepsState.templeExploration.phase, "footsteps");
assert.equal(footstepsState.templeExploration.elapsed, 7);
...
assert.ok(roadVisual.routeNodeCount >= 2);
...
const shenRouteSave = JSON.parse(await evaluate(`localStorage.getItem("wudao-high-martial-v1")`));
shenRouteSave.completedTempleTasks = [...new Set([...(shenRouteSave.completedTempleTasks || []), "shen_promise"])];
await reloadWithSave(shenRouteSave);
...
assert.match(await text(), /医术一、悟性三、阅历五百方可入门/);
assert.match(await text(), /阅历\s*1595/);
await click("breakthrough-five-animals");
await click("choose-five-aspect", "ape");
await click("shen-daily-action", "qingqing");
await click("shen-daily-action", "five_animals");
...
assert.equal(saved.potential, 1109);
...
assert.equal(migratedVersionSeven.version, 12);
...
assert.equal(debugSnapshot.saveVersion, 12);
