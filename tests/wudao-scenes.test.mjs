...
test("nine compressed environment assets include both authored origin openings and the east-road encounter", () => {
  assert.equal(SCENE_ASSET_PATHS.length, 9);
  assert.equal(new Set(SCENE_ASSET_PATHS).size, 9);
  ...
  assert.ok(SCENE_ASSET_PATHS.some((asset) => asset.endsWith("east-road-porter-v1.webp")));
});

test("every visual scene exposes serializable, bounded and uniquely identified markers", () => {
  for (const screen of ["shenOriginArrival", "streetOriginMarket", "eastRoadPorter", "templeTasks", "roadTrial", "shenArrival", "danObservation", "yanJinghongArrival", "wangBattle"]) {
