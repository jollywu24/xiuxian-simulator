...
发布以可独立试玩和回滚的检查点为单位。同一界面或纵切的连续修正先在本地用相关检查和受影响浏览器套件收敛，不逐次提交部署；交互语义变化先同步所有依赖旧行为的断言。准备发布且局部检查通过后运行一次完整门禁，不用全量门禁逐个发现旧断言。

用户已经要求发布，或当前`/goal`明确包含发布时，完成门禁后继续推送、部署和线上核验，无需重复询问。本地检查点或不含发布的目标完成不自动授权推送`main`；只要求提交时不自动推送。

只暂存本次范围文件，不提交无关截图、日志、浏览器配置或用户素材。可选战斗预览保持独立路径，不替换正式根页面；普通入口不得显示调试控件或注入仅用于`?debug=1`回归的`window.WudaoDebug`。
...
部署产物在上传前还会从 `tools/sandbox-s0/`、`data/sandbox/` 和权威美术源生成 `web/sandbox-s0/`。该目录只存在于部署产物，不取代 Pages 根路径的正式游戏。
