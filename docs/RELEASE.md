...
发布以“可试玩检查点”为单位，不以每次文案、CSS或局部规则修改为单位。同一界面或同一纵切的连续修正先保留在本地，使用`verify:quick`和受影响浏览器套件收敛；交互语义变化必须先全局审计并一次性更新依赖旧行为的回归步骤。只有全部受影响套件通过后才运行一次完整发布门禁；不得把完整门禁当作逐个发现旧断言的调试循环。只有用户明确要求发布、完成当前`/goal`、或已经形成可独立试玩和回滚的检查点时才推送`main`。
